      ...
      <Link className="contents" to={`/category/${listing.type}/${id}`}>
        ...
        <div className="w-full p-[10px">
          <div className="flex items-center space-x-1">
            ...
            <p
              className="font-semibold text-sm mb-[2px] text-gray-600
            truncate"
            >
              {listing.address}
            </p>
          ...
          <p className="text-[#457b9d] mt-2 font-semibold">
            {/* if listing has offer, show discountedPrice, else show regularPrice */}
            $
            {listing.offer
              ...
              : listing.regularPrice
                  .toString()
                  .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
            {/* if listings is rent show /month */}
            {listing.type === "rent" && " / month"}
          </p>
          <div className="flex items-center mt-[10px] space-x-3">
            <div className="flex items-center space-x-1">
              {/* if listing has more than 1 bedrooms, show the number bedrooms with beds, else show 1 bed */}
